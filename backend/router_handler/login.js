...
exports.login = (req, res) => {
  ...
  if (/[`~!@#$%^&*()_\-+=<>?:"{}|,\/;\[\]·~！@#￥%……&*（）——\-+={}|《》？：“”【】、；‘’，。、]/g.test(user_info.user_name)) {
    return res.cc("禁止使用特殊字符搞些歪门邪道");
  }

  const sqlSearch = "select * from user_info where user_name = ?";
  db.query(sqlSearch, user_info.user_name, (err, results) => {
    if (err) return res.cc(err);
    if (results.length !== 1) return res.cc("登录失败");
    // 判断密码是否正确
    const compareRes = bcrypt.compareSync(user_info.pass_word, results[0].pass_word);
